              ...
              <div className="p-4">
                {pastCrops.map((crop, index) => (
                  <div key={index} className="mb-4 p-4 border border-green-50 rounded-3xl relative bg-green-50/30 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-1">
                      <div>
                        <label className="block text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">Crop Grown</label>
                        <input
                          type="text"
                          required
                          placeholder="e.g., Tomato, බඩඉරිගු"
                          className="text-black w-full px-3 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-green-400 text-xs font-medium bg-white transition-colors"
                          value={crop.cropName}
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">Time Period Grown</label>
                        <div className="flex flex-wrap items-center gap-1">
                          <select
                            required
                            className="text-black flex-1 min-w-[60px] px-1 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-green-400 bg-white text-[10px] font-medium cursor-pointer"
                          ...
                          <select
                            required
                            className="text-black flex-1 min-w-[55px] px-1 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-green-400 bg-white text-[10px] font-medium cursor-pointer"
                          ...
                          <span className="text-gray-400 text-[10px] font-bold px-0.5">-</span>
                          <select
                            required
                            className="text-black flex-1 min-w-[60px] px-1 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-green-400 bg-white text-[10px] font-medium cursor-pointer"
                            value={crop.endMonth}
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">Fertilizers (Separate by commas)</label>
                        <input
                          type="text"
                          placeholder="e.g., යූරියා, Compost, NPK"
                          className="text-black w-full px-3 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-green-400 text-xs font-medium bg-white transition-colors"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">Pesticides (Separate by commas)</label>
                        <input
                          type="text"
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-2">
                  <button
                    ...
                    className="w-full sm:w-auto text-[11px] font-extrabold text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100 px-4 py-2.5 rounded-full transition-colors flex justify-center items-center"
                  >
                    + ADD PAST CROP
                  </button>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                    <select
                      ...
                      className="px-4 py-2.5 border border-gray-300 rounded-full outline-none focus:ring-2 focus:ring-green-500 bg-white text-gray-700 text-xs font-bold w-full sm:w-auto cursor-pointer"
                    >
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full sm:w-auto px-6 py-2.5 border border-transparent text-xs font-extrabold rounded-full shadow-sm text-white bg-green-600 hover:bg-green-700 disabled:bg-green-300 disabled:cursor-not-allowed transition-all uppercase tracking-wide"
                    >
                      {loading ? 'Please wait...' : 'Process'}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-6 space-y-4 animate-fade-in-up">
              ...
              <div className={`bg-white shadow-sm rounded-3xl p-4 border relative overflow-hidden ${evaluation.targetEvaluation.isSuitable ? 'border-green-100' : 'border-red-100'}`}>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-3 gap-2">
                  <h3 className="text-[11px] font-extrabold text-gray-500 leading-snug uppercase tracking-widest">
                  ...
                  <span className={`px-3 py-1 rounded-full text-[9px] uppercase tracking-widest font-extrabold text-white shadow-sm ${evaluation.targetEvaluation.isSuitable ? 'bg-green-500' : 'bg-red-500'}`}>
                ...
                <div className="bg-gray-50 rounded-2xl p-3 border border-gray-100">
                  <p className="text-[10px] font-extrabold text-gray-400 mb-1.5 uppercase tracking-widest">
                    {language === 'Sinhala' ? 'හේතුව:' : 'Feedback:'}
                  </p>
                  <ul className="list-disc list-inside space-y-1 text-gray-600 text-[11px] font-medium">
              ...
              <div className="bg-white shadow-sm rounded-3xl p-4 border border-amber-100 relative overflow-hidden">
                ...
                <h3 className="text-[11px] font-extrabold text-gray-500 mb-2 uppercase tracking-widest">
                  {language === 'Sinhala' ? 'පසෙහි පෝෂක විශ්ලේෂණය' : 'Soil Nutrient Analysis'}
                </h3>
                <p className="text-amber-800 text-[11px] font-bold mb-3 bg-amber-50 rounded-xl p-2.5">
                  {evaluation.soilCondition.status}
                </p>
                
                {/* Table 1: Current Soil Condition */}
                <div className="overflow-x-auto mb-4 border border-gray-100 rounded-xl">
                  <table className="min-w-full text-left text-[10px] sm:text-[11px]">
                    <thead className="bg-amber-50 text-amber-900 font-bold uppercase tracking-wider">
                      <tr>
                        <th className="px-3 py-2 border-b border-amber-100">{language === 'Sinhala' ? 'පෝෂකය' : 'Nutrient'}</th>
                        <th className="px-3 py-2 border-b border-amber-100">{language === 'Sinhala' ? 'මට්ටම' : 'Level'}</th>
                        <th className="px-3 py-2 border-b border-amber-100">{language === 'Sinhala' ? 'අනාවැකිය' : 'Depletion Prediction'}</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-50 text-gray-600 font-medium">
                      {evaluation.soilNutrientLevels?.map((row, idx) => (
                        <tr key={idx}>
                          <td className="px-3 py-2 font-bold text-gray-700">{row.nutrient}</td>
                          <td className="px-3 py-2">{row.level}</td>
                          <td className="px-3 py-2">{row.depletionPrediction}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                {/* Table 2: Required Nutrients */}
                <h3 className="text-[10px] font-extrabold text-gray-400 mb-2 uppercase tracking-widest">
                  {language === 'Sinhala' ? 'අවශ්‍ය පෝෂක' : 'Required Additions'}
                </h3>
                <div className="overflow-x-auto border border-gray-100 rounded-xl">
                  <table className="min-w-full text-left text-[10px] sm:text-[11px]">
                    <thead className="bg-green-50 text-green-900 font-bold uppercase tracking-wider">
                      <tr>
                        <th className="px-3 py-2 border-b border-green-100">{language === 'Sinhala' ? 'පෝෂකය' : 'Nutrient'}</th>
                        <th className="px-3 py-2 border-b border-green-100">{language === 'Sinhala' ? 'ප්‍රමාණය' : 'Amount'}</th>
                        <th className="px-3 py-2 border-b border-green-100">{language === 'Sinhala' ? 'නිර්දේශිත මූලාශ්‍රය' : 'Recommended Source'}</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-50 text-gray-600 font-medium">
                      {evaluation.requiredNutrients?.map((row, idx) => (
                        <tr key={idx}>
                          <td className="px-3 py-2 font-bold text-gray-700">{row.nutrient}</td>
                          <td className="px-3 py-2">{row.amount}</td>
                          <td className="px-3 py-2">{row.recommendedSource}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {!evaluation.targetEvaluation.isSuitable && (
                <div className="bg-white shadow-sm rounded-3xl p-4 border border-blue-100 relative overflow-hidden">
                  ...
                  <h3 className="text-[11px] font-extrabold text-gray-500 mb-3 uppercase tracking-widest">
                    {language === 'Sinhala' ? 'නිර්දේශිත විකල්ප වගාවන් (අවම වශයෙන් 3)' : 'Recommended Alternatives (Min. 3)'}
                  </h3>
                  <div className="grid grid-cols-1 gap-2">
                    {evaluation.alternativeSuggestions.map((item, idx) => (
                      <div key={idx} className="bg-blue-50/50 rounded-2xl p-3 border border-blue-50">
                        <h4 className="text-[11px] font-extrabold text-blue-900 mb-1">{item.cropName}</h4>
                        <ul className="list-disc list-inside text-[10px] text-blue-800/80 space-y-1 font-medium ml-1">
